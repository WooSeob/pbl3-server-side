...
async function getBufferFeedbacks(queryKeyword){
  ...
  if(cBuffer != null){
    for(let recommend of cBuffer.recommended){
      FeedBacks.push({
        target: recommend.category,
        Weights: { feedbackWeight: Math.pow(1.3, -recommend.feedback) },
      })
    }
  }
...
async function searchOptimization(type, isMatched, queriedKeyword, recommeded) {
  ...
  if (isMatched) {
    ...
    for (let keyword of recommeded.category.keywords) {
      if (keyword.key == queriedKeyword) {
        ...
        feedback = {
          ...
          Weights: { feedbackWeight: Math.pow(1.3, -keyword.feedback) },
        };
      }
    }
    ...
    for (let rCategory of buffer.recommended) {
      ...
      FeedBacks.push({
        target: recommeded.category,
        Weights: { feedbackWeight: Math.pow(1.3, -rCategory.feedback) },
      })
    ...
    if (Matching.isMatched) {
      ...
      //추천목록이 5개 이상이면
      if(buffer.recommended.length > 3){
        // 새로운 카테고리를 만든다.
        await KeywordMatchingEngine.addKeywordsToProperCategory(Matching);
        buffer.keyword = ""
        await buffer.save()
